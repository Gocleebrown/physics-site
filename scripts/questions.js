  ...
  question: function() {
    ...
    dims.forEach((d,i) => {
      if (pick.includes(i)) {
        ...
        d.valDisplay = d.val.toFixed(i===0?4:4);  // keep same DP
        d.uncDisplay = d.unc.toFixed(i===0?4:5);
        d.unitDisplay = "m";
      }
    });

    // --- Calculations (always in SI metres/kg) ---
...
Percentage uncertainty = ${percentageDensityUnc}%`,
          explanation:
            "Sum the fractional uncertainties: mass fraction + volume fraction, "
            + "then ×100 to get percentage uncertainty.",
          marks: [
        ...
        },
        {
          ...
          explanation:
            "Systematic errors shift all measurements by the same amount, "
            + "e.g. a zero offset on the balance or caliper.",
          marks: [
